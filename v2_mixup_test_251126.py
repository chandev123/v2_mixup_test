...
def save_mixup_snapshot(orig_x, orig_y, mixed_x, mixed_y, mean, std, num_samples=5):
    ...
    plt.tight_layout()
    plt.savefig('mixup_training_snapshot.png')
    plt.close()
    print("훈련 중 Mixup 스냅샷 저장 완료: mixup_training_snapshot.png")
...
def visualize_mixup_samples(loader, mixup_fn, mean, std, num_samples=5):
    ...
    plt.tight_layout()
    plt.savefig('mixup_samples_comparison.png')
    plt.show()
    print("Mixup 비교 이미지 저장 완료: mixup_samples_comparison.png")
